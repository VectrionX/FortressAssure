...
export const Dashboard: React.FC<DashboardProps> = ({ data, status, onOpenEvidence }) => (
  <div className="space-y-6 sm:space-y-8">
    <section className="rounded-3xl bg-slate-900 p-6 text-white shadow-xl sm:p-8">
      <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-300">FortressAssureX · evidence register</p>
    ...
    <section className={`rounded-2xl border p-5 ${statusStyle[status.code]}`} aria-label="Assessment status">
      <p className="text-xs font-black uppercase tracking-[0.16em] opacity-70">Assessment status</p>
      <h3 className="mt-1 text-xl font-bold">{status.label}</h3>
      <p className="mt-2 max-w-4xl text-sm leading-6">{status.detail}</p>
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
        {SUPPORTED_MVP_MODULES.map(module => <span key={module} className={`rounded-full px-3 py-1 ${status.coveredModules.includes(module) ? 'bg-cyan-700 text-white' : 'bg-white/70 text-slate-700 ring-1 ring-inset ring-slate-300'}`}>{module}: {status.coveredModules.includes(module) ? 'evidence recorded' : 'not assessed'}</span>)}
      </div>
    </section>

    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      {findingCounts(data.findings).map(({ level, count }) => <div key={level} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><RiskBadge level={level as RiskLevel} /><div className="mt-3 text-3xl font-bold text-slate-900">{count}</div><p className="text-xs text-slate-500">human-recorded findings</p></div>)}
    </section>

    <section className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-lg font-bold text-slate-900">Current MVP boundary</h3><p className="mt-1 text-sm text-slate-600">Only the modules below accept evidence-backed human findings. Every other domain remains explicitly unsupported.</p></div><button onClick={onOpenEvidence} className="rounded-lg bg-cyan-700 px-4 py-2 text-sm font-bold text-white hover:bg-cyan-800">Record evidence</button></div>
